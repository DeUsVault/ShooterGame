...
void AShooterPlayerController::SetPlayer( UPlayer* InPlayer )
{
	...
	{
		// Epic used to Create Menu Here.
		// But it's bad, so i'm not doing that :)
...
void AShooterPlayerController::InitPlayerState()
{
	...
	{
		const AGameModeBase* GameMode = GetWorld() ? GetWorld()->GetAuthGameMode() : nullptr;
		...
		if (GameMode)
		{
			...
			check(!UShooterGameData::Get().PlayerStateClass.IsNull()); // No PlayerStateClass set under Game Data.
			PlayerState = GetWorld()->SpawnActor<APlayerState>(UShooterGameData::Get().PlayerStateClass.LoadSynchronous(), SpawnInfo);
