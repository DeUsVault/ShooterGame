...
void AShooterCharacter::PawnClientRestart()
{
	...
	if (APlayerController* PC = Cast<APlayerController>(GetController()))
	{
		if (auto* Subsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(PC->GetLocalPlayer()))
		{
			...
			UInputMappingContext* DevContext = UShooterGameData::Get().DeveloperInputMappingContext.LoadSynchronous();
			Subsystem->AddMappingContext(DevContext, 0);
#endif
		}
	}
...
void AShooterCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	UInputAction* JumpAction = UShooterGameData::Get().InputActionJump.LoadSynchronous();
	...
	UInputAction* FireAction = UShooterGameData::Get().InputActionFire.LoadSynchronous();
	EnhancedInputComponent->BindAction(FireAction, ETriggerEvent::Triggered, this, &ThisClass::OnStartFire);
	EnhancedInputComponent->BindAction(FireAction, ETriggerEvent::Completed, this, &ThisClass::OnStopFire);
	// @TODO: Remove Trigger Fire, we can do this with deadzones now.

	UInputAction* AimDownSightsAction = UShooterGameData::Get().InputActionAimDownSights.LoadSynchronous();
	EnhancedInputComponent->BindAction(AimDownSightsAction, ETriggerEvent::Triggered, this, &ThisClass::OnStartTargeting);
	EnhancedInputComponent->BindAction(AimDownSightsAction, ETriggerEvent::Completed, this, &ThisClass::OnStopTargeting);

	UInputAction* CycleWeaponAction = UShooterGameData::Get().InputActionCycleWeapon.LoadSynchronous();
	EnhancedInputComponent->BindAction(CycleWeaponAction, ETriggerEvent::Triggered, this, &ThisClass::OnCycleWeapon);

	UInputAction* ReloadAction = UShooterGameData::Get().InputActionReload.LoadSynchronous();
	EnhancedInputComponent->BindAction(ReloadAction, ETriggerEvent::Triggered, this, &ThisClass::OnReload);

	UInputAction* SprintAction = UShooterGameData::Get().InputActionSprint.LoadSynchronous();
	EnhancedInputComponent->BindAction(SprintAction, ETriggerEvent::Triggered, this, &ThisClass::OnStartRunning);
	EnhancedInputComponent->BindAction(SprintAction, ETriggerEvent::Completed, this, &ThisClass::OnStopRunning);

	// @TODO: Make case for toggle sprint ig?

#if !UE_BUILD_SHIPPING // Developer only keybinds.
	UInputAction* SuicideAction = UShooterGameData::Get().InputActionSuicide.LoadSynchronous();
	EnhancedInputComponent->BindAction(SuicideAction, ETriggerEvent::Triggered, GetController(), "Suicide");
#endif
}
...
void AShooterCharacter::Move(const FInputActionValue& Value)
{
	...
	{ // Strafe Input (Left / Right)
		AddMovementInput( 
		    FQuatRotationMatrix(GetActorQuat()).GetScaledAxis(EAxis::Y),
		    FMath::Clamp(MoveInput.X, -1.f, 1.f)
		);
	}

	{ // Forward Input (Forward / Back)
	...
	{ // Vertical Input (Up / Down) - Flying Only
...
void AShooterCharacter::Look(const FInputActionValue& Value)
{
	...
	AddControllerPitchInput(LookInput.Y * BaseLookUpRate * GetWorld()->GetDeltaSeconds());
}
...
void AShooterCharacter::OnCycleWeapon(const FInputActionValue& Value)
{
	float CycleInput = Value.Get<float>();
	AShooterPlayerController* MyPC = Cast<AShooterPlayerController>(Controller);
	if (MyPC && MyPC->IsGameInputAllowed())
	{
		if (Inventory.Num() >= 2 && (!CurrentWeapon || CurrentWeapon->GetCurrentState() != EWeaponState::Equipping))
		{
			const int32 CurrentWeaponIdx = Inventory.IndexOfByKey(CurrentWeapon);
			AShooterWeapon* NextWeapon = Inventory[(CycleInput > 0 ? 
				(CurrentWeaponIdx + 1) : 					// Next Weapon
				(CurrentWeaponIdx - 1 + Inventory.Num())) 	// Prev Weapon
				% Inventory.Num()];
